...
```{r}
setEPS()
postscript("temporal/number_of_terms_polynomial.eps", width = 6, height = 3)
plot_N_p_Q
dev.off()
```
...
```{r}
setEPS()
postscript("temporal/partitions_time_and_size_full.eps", width = 6, height = 3)
plot_full
dev.off()
```
...
df <- as.data.frame(cbind(p_results,Q_results,time1,size_partitions))
names(df) <- c("p","Q","time","size")
...
```{r}
setEPS()
postscript("temporal/partitions_time_and_size_equivalent.eps", width = 6, height = 3)
plot_equivalent
dev.off()
```

Otros posibles plots: poner al lado time de los 2 y size de los 2 para comparar, pero tendria que ser misma escala.

Ponerlos juntos con escala magma en otros?
